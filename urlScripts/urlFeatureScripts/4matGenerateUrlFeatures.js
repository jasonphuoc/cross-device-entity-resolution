...
async.forEachLimit(contentArray, 1, function(line, callback) {
  ...
  scriptManager.run('generateUrlFeatures', [uid1], [uid2, 'MATCHING'], function(err, result) {
    if(err) throw err;
    for(let i=0; i<5; i++)
      file.appendFileSync("./../features/pairs_for_matching_url_l" + (i+1) + ".csv", uid1 + "," + uid2 + "," + result[i] + '\n');
    count++;
    if(count % 100 == 0)
